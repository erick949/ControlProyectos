/**
 * ══════════════════════════════════════════════════════
 *  src/pages/MisProyectos.jsx  ← VERSIÓN CON API
 * ══════════════════════════════════════════════════════
 *
 * Cambios:
 * - Ya no recibe `proyectos` como prop.
 * - Carga proyectos propios desde la API al montar.
 * - El enlace del PDF apunta a la URL real del servidor.
 */
...
import { getProyectos } from '../api/proyectos'   // ← NUEVO

export default function MisProyectos({ session }) {
  ...
  const filtrados = useMemo(
    () => proyectos.filter((p) => estadoFiltro === 'Todos' || p.estado === estadoFiltro),
          ...
          {['Todos', 'Activo', 'Inactivo'].map((e) => (
            <button key={e} type="button" onClick={() => setEstadoFiltro(e)}
              className={`text-[12px] font-semibold px-3.5 py-1.5 rounded-full border transition-colors ${
                estadoFiltro === e ? 'bg-primary text-white border-primary' : 'text-neutral border-border hover:border-primary hover:text-primary'
              }`}>
              {e}
            </button>
          ))}
        ...
        {loading ? (
          ...
          <div className="text-center py-14">
            <p className="text-[13px] text-neutral mb-4">
              {proyectos.length === 0 ? 'Todavía no has registrado ningún proyecto.' : 'No hay proyectos con ese estado.'}
            </p>
            {proyectos.length === 0 && (
              <Button variant="primary" onClick={() => navigate('/proyecto/nuevo')}>Registrar mi primer proyecto</Button>
            )}
          </div>
        ) : (
          <div className="flex flex-col divide-y divide-border">
            {filtrados.map((p) => (
              <div key={p.id} className="py-4 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6">
                ...
                {/* ── CAMBIO: href real al PDF del backend ── */}
                {p.pdf_url && (
