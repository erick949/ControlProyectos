/**
 * src/pages/Login.jsx  ← VERSIÓN CON API
 *
 * Cambios respecto al original:
 * - handleSubmit llama a login() de api/auth.js en lugar de comparar con ADMIN_CREDENTIALS.
 * - Los errores de validación del backend se muestran en generalError.
 * - Se añade estado `loading` para deshabilitar el botón mientras espera.
 * - Se elimina la importación de ADMIN_CREDENTIALS.
 */

import { useState } from 'react'
import { useNavigate, Link } from 'react-router-dom'
import { login } from '../api/auth'   // ← NUEVO

// (Los íconos y DOTS se mantienen exactamente igual que en el original)
const iconBase = { fill: 'none', viewBox: '0 0 24 24', strokeWidth: 1.75, stroke: 'currentColor' }
const IconUser = (p) => (<svg {...iconBase} {...p}><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.5 20.25a8.25 8.25 0 0115 0" /></svg>)
const IconLock = (p) => (<svg {...iconBase} {...p}><path strokeLinecap="round" strokeLinejoin="round" d="M16.5 10.5V7.5a4.5 4.5 0 10-9 0v3M5.25 10.5h13.5a1 1 0 011 1v8.5a1 1 0 01-1 1H5.25a1 1 0 01-1-1v-8.5a1 1 0 011-1z" /></svg>)
const IconEye = (p) => (<svg {...iconBase} {...p}><path strokeLinecap="round" strokeLinejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" /><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>)
const IconEyeOff = (p) => (<svg {...iconBase} {...p}><path strokeLinecap="round" strokeLinejoin="round" d="M3.98 8.223A10.477 10.477 0 001.934 12c1.292 4.338 5.31 7.5 10.066 7.5.993 0 1.953-.138 2.863-.395M6.228 6.228A10.45 10.45 0 0112 4.5c4.756 0 8.774 3.162 10.066 7.5a10.522 10.522 0 01-4.293 5.774M6.228 6.228L3 3m3.228 3.228l3.65 3.65m7.894 7.894L21 21m-3.228-3.228l-3.65-3.65m0 0a3 3 0 10-4.243-4.243m4.242 4.243L9.88 9.88" /></svg>)
...
export default function Login({ onLogin }) {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true)
    try {
      // login() llama a POST /api/auth/login/ y guarda los tokens en localStorage
      const session = await login(form.usuario, form.contrasena)
      onLogin?.(session)
      navigate('/dashboard')
    } catch (err) {
      // El backend devuelve { detail: "Usuario o contraseña incorrectos." }
      setGeneralError(err.data?.detail || err.data?.non_field_errors?.[0] || 'Error al iniciar sesión.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const inputBase = 'w-full h-11 rounded-lg bg-white border border-[#e2e2ec] pl-9 pr-3 text-[13px] text-[#1e1b2e] placeholder-[#a3a3b2] outline-none transition-colors focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/15'
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-gradient-to-br from-[#fbfbfe] via-[#f4f4f9] to-[#eef0f7] flex items-center justify-center p-4">
      ...
      {DOTS.map((d, i) => (
        <span key={i} className={`pointer-events-none absolute w-1 h-1 rounded-full ${d.color}`} style={{ top: d.top, left: d.left }} />
      ))}
      ...
      <div className="relative w-full max-w-[400px] mt-12 rounded-2xl border border-indigo-500/15 bg-white/90 px-9 pb-9 pt-12 text-[#1e1b2e] shadow-[0_20px_50px_rgba(30,27,75,0.10),0_2px_8px_rgba(30,27,75,0.06)] backdrop-blur-xl flex flex-col items-center">
        <div className="absolute -top-12 left-1/2 -translate-x-1/2 z-10 flex flex-col items-center">
          <img src="/logo.png" alt="Logo Proyecta" className="w-24 h-24 object-contain drop-shadow-[0_10px_15px_rgba(30,27,75,0.25)]" />
        </div>

        <div className="text-center mb-6 mt-2">
          <p className="text-[10px] text-indigo-600 tracking-[0.25em] uppercase font-bold mt-1 mb-5">Control de Proyectos</p>
        ...
        <form onSubmit={handleSubmit} noValidate className="w-full flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative flex items-center">
              ...
              <input type="text" placeholder="nombre@empresa.com" value={form.usuario}
          ...
          {generalError && (
            <p className="text-xs text-red-600 text-center bg-red-50 py-1.5 rounded border border-red-200">{generalError}</p>
          )}

          {/* ── CAMBIO: disabled y texto mientras carga ── */}
          <button type="submit" disabled={loading}
            className="mt-3 w-full py-3 rounded-full bg-gradient-to-r from-indigo-500 to-indigo-700 text-white font-extrabold text-[13px] uppercase tracking-widest shadow-[0_8px_20px_rgba(79,70,229,0.28)] transition-all hover:shadow-[0_10px_28px_rgba(99,102,241,0.38)] hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-60 disabled:cursor-not-allowed">
            {loading ? 'Ingresando...' : 'Iniciar Sesión'}
          </button>

          <div className="text-center mt-1">
            <Link to="#" className="text-[13px] text-[#5b5b6b] hover:text-indigo-600 underline underline-offset-2 transition-colors">
